...
const createProduct = async (req, res) => {
    try {
        ...
        if (!ownerId)
            return;
        const product = await productModel_1.Product.create({ ...req.body, owner: ownerId });
...
const getProducts = async (req, res) => {
    try {
        ...
        if (!ownerId)
            return;
        const products = await productModel_1.Product.find({ owner: ownerId }).sort({ createdAt: -1 });
        res.json(products);
    }
    catch (err) {
...
const getProductById = async (req, res) => {
    try {
        ...
        const product = await productModel_1.Product.findOne({ _id: req.params.id, owner: ownerId });
        if (!product)
...
const updateProduct = async (req, res) => {
    try {
        ...
        if (!(0, ids_1.isValidObjectId)(req.params.id))
            return res.status(400).json({ message: 'Invalid id' });
        const product = await productModel_1.Product.findOneAndUpdate({ _id: req.params.id, owner: ownerId }, req.body, { new: true, runValidators: true });
        if (!product)
            return res.status(404).json({ message: 'Product not found' });
        res.json(product);
    }
...
const deleteProduct = async (req, res) => {
    try {
        ...
        if (!(0, ids_1.isValidObjectId)(req.params.id))
            return res.status(400).json({ message: 'Invalid id' });
        const product = await productModel_1.Product.findOneAndDelete({ _id: req.params.id, owner: ownerId });
        if (!product)
            return res.status(404).json({ message: 'Product not found' });
