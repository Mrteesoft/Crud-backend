...
const createPurchase = async (req, res) => {
    try {
        ...
        if (!ownerId)
            return;
        const purchase = await purchase_model_1.Purchase.create({ ...req.body, createdBy: ownerId });
        const installments = (0, installments_1.buildInstallments)(purchase);
        await installment_model_1.Installment.insertMany(installments);
...
const getPurchases = async (req, res) => {
    try {
        ...
        if (status)
            query.status = status;
        const purchases = await purchase_model_1.Purchase.find(query).sort({ createdAt: -1 });
        res.json(purchases);
    }
    catch (err) {
...
const getPurchaseById = async (req, res) => {
    try {
        ...
        const purchase = await purchase_model_1.Purchase.findOne({ _id: req.params.id, createdBy: ownerId });
        if (!purchase)
...
const updatePurchase = async (req, res) => {
    try {
        ...
        if (!(0, ids_1.isValidObjectId)(req.params.id))
            return res.status(400).json({ message: 'Invalid id' });
        const purchase = await purchase_model_1.Purchase.findOneAndUpdate({ _id: req.params.id, createdBy: ownerId }, req.body, { new: true, runValidators: true });
        if (!purchase)
            return res.status(404).json({ message: 'Purchase not found' });
        res.json(purchase);
    }
...
const deletePurchase = async (req, res) => {
    try {
        ...
        if (!(0, ids_1.isValidObjectId)(req.params.id))
            return res.status(400).json({ message: 'Invalid id' });
        const purchase = await purchase_model_1.Purchase.findOneAndDelete({ _id: req.params.id, createdBy: ownerId });
        if (!purchase)
            return res.status(404).json({ message: 'Purchase not found' });
        await installment_model_1.Installment.deleteMany({ purchaseId: purchase._id });
...
const getInstallments = async (req, res) => {
    try {
        ...
        if (!purchase)
            return res.status(404).json({ message: 'Purchase not found' });
        const installments = await installment_model_1.Installment.find({ purchaseId: purchase._id }).sort({ sequence: 1 });
...
const payNextInstallment = async (req, res) => {
    try {
        ...
        if (!purchase)
            return res.status(404).json({ message: 'Purchase not found' });
        const next = await installment_model_1.Installment.findOne({ purchaseId: purchase._id, status: 'pending' }).sort({ sequence: 1 });
        if (!next)
            return res.status(400).json({ message: 'All installments already paid' });
        next.status = 'paid';
        await next.save();
        const remaining = await installment_model_1.Installment.countDocuments({ purchaseId: purchase._id, status: 'pending' });
        if (remaining === 0) {
